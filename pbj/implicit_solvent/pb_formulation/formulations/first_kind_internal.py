...
def calderon_interior_operator_preconditioner(solute):
    solute.matrices["preconditioning_matrix"] = solute.matrices["A_int"]
    apply_calderon_precondtioning(solute)


def calderon_exterior_operator_scaled_preconditioner(solute):
    solute.matrices["preconditioning_matrix"] = solute.matrices["A_ext_scal"]
    apply_calderon_precondtioning(solute)
...
def exterior_mass_matrix(matrix, ep_ex, ep_in):
    from bempp_cl.api.utils.helpers import get_inverse_mass_matrix
    from bempp_cl.api.assembly.blocked_operator import BlockedDiscreteOperator
...
def calderon_squared_lowered_parameters_preconditioner(solute):
    from pbj.implicit_solvent.utils import rhs_to_discrete_form
